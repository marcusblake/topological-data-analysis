...
class ConvolutionalAutoencoder(AutoencoderBase):
    def __init__(self, latent_space_dim: int = 2):
        super().__init__(latent_space_dim)
        self.encoder =nn.Sequential(
            nn.Conv2d(1, 96, kernel_size=3, stride=1, padding=1), # Maintains input dimensions 28x28. Include padding at input layer.
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2), # Downsamples to 14x14
            nn.Conv2d(96, 32, kernel_size=3, stride=1, padding=1), # Maintins input dimension 14x14
            nn.ReLU(),
            ...
            nn.Flatten(), # Flappens to vector of 3x3x8 since the number of filters from last conv2d layer is 8
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose2d(32, 96, kernel_size=2, stride=2), # Upsamples to 14x14
            nn.ConvTranspose2d(96, 1, kernel_size=2, stride=2) # Upsamples to 28x28
        )
